...
def generate_mask(img, width=4, mask_type='random'):
    ...
    rd_pair_idx += torch.arange(start=0,
                                end=n * h // width * w // width * width**2, # バッチサイズを考慮した全ピクセル数
...
def interpolate_mask(tensor, mask, mask_inv):
    ...
    kernel = kernel[np.newaxis, np.newaxis, :, :]  # np.newaxis: 配列を引き延ばして、サイズが違う配列同士でも演算をできるようにする
...
for epoch in range(epoch_init, opt.n_epoch + 1):
    ...
    for iteration, clean in enumerate(TrainingLoader):
        ...
        net_input, mask = masker.train(noisy)   # masker.train(): Global Masker、net_input: 図のΩ_y、mask: Global Mask Mapper
        noisy_output = network(net_input)
        n, c, h, w = noisy.shape
        noisy_output = (noisy_output*mask).view(n, -1, c, h, w).sum(dim=1)
        diff = noisy_output - noisy  # 図のh(f_θ(Ω_y))
