...
parser.add_argument('--test_gt_dirs', type=str, default='./data/validation/ground_truth')
...
# ノイズ付きテスト画像のGTをPILで開いて、numpyに変換して計算しやすくする
def test_GT(dataset_dir):
    # 指定したディレクトリdataset_dirの中にある、すべてのファイル・フォルダのパスをリストとして取得
...
# bsd500をPILで開いて、numpyに変換して計算しやすくする
def validation_bsd500(dataset_dir):
    fns = glob.glob(os.path.join(dataset_dir, "*"))  # glob.glob(): 条件を満たすパスの文字列を要素とするリストを取得
    fns.sort()    # 取得したファイルのリストを昇順（アルファベット順）にソート
    images = []
    for fn in fns:
...
coco2017_dir = os.path.join(opt.test_dirs)
test_GT_dir  = os.path.join(opt.test_gt_dirs)
BSD500_dir   = os.path.join(opt.test_dirs)
"""
valid_dict = {
...
valid_dict = {
    ...
    # "Kodak24": validation_kodak(Kodak_dir),
    # "BSD500": validation_bsd500(BSD500_dir),
}

# PSNR計算用に、ノイズ付きテスト画像のGTを用意
test_GT = test_GT(test_GT_dir)
...
for valid_name, valid_images in valid_dict.items():
    ...
    for i in range(repeat_times):
        for idx, im in enumerate(valid_images):
            # ノイズ付きテスト画像のGTを1枚ずつ取り出す
            origin255 = test_GT[idx].copy()
